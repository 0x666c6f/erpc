...
func contextFrom(ctxOrExec any) context.Context {
	if c, ok := ctxOrExec.(context.Context); ok && c != nil {
		return c
	}
	if exec, ok := ctxOrExec.(interface{ Context() context.Context }); ok && exec.Context() != nil {
		return exec.Context()
	}
...
func resultToJsonRpcResponse(result *common.NormalizedResponse, ctxOrExec any) *common.JsonRpcResponse {
	if result == nil {
		return nil
	}
	ctx := contextFrom(ctxOrExec)
...
func classifyAndHashResponse(r *execResult, ctxOrExec any, config *config) {
...
func resultOrErrorToHash(r *execResult, ctxOrExec any, config *config) (string, error) {
	ctx := contextFrom(ctxOrExec)
	if r.Err != nil {
		if isConsensusValidError(r.Err) || isAgreedUponError(r.Err) {
