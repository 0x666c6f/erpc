...
// These tests pin the rollback behavior of the tracker's block-head state:
...
// routing decision derived from it — until process restart.
//
// Fork semantics (differs from upstream's tolerance-band approach): every
// per-upstream correction is stored immediately, and the NETWORK head is
// protected by rollback hysteresis instead — a small (≤2 block) drop of the
// derived max is held until a second consistent observation confirms it,
// while larger drops apply immediately. The network head is never lowered by
...
func TestTrackerLatestBlockRollbackTolerance(t *testing.T) {
	...
	t.Run("SmallRollbackAcceptedWithNetworkHysteresis", func(t *testing.T) {
		tracker := newRollbackTestTracker(t, "test-rollback-small")
		ups := common.NewFakeUpstream("a")

		tracker.SetLatestBlockNumber(ups, 10_000, 0)
		// Per-upstream corrections are stored immediately; the network head
		// holds a small (≤2 block) drop until a second consistent
		...
		assert.Equal(t, int64(1), blockHeadLag(tracker, ups))

		// Second consistent observation confirms the network rollback.
		tracker.SetLatestBlockNumber(ups, 9_999, 0)
		assert.Equal(t, int64(9_999), networkLatest(tracker, ups.NetworkId()))
		assert.Equal(t, int64(0), blockHeadLag(tracker, ups))
	})
...
func TestTrackerFinalizedBlockRollbackTolerance(t *testing.T) {
	t.Run("SmallRollbackAcceptedWithNetworkHysteresis", func(t *testing.T) {
		...
		tracker.SetFinalizedBlockNumber(ups, 9_999)

		assert.Equal(t, int64(9_999), upstreamFinalized(tracker, ups))
		assert.Equal(t, int64(10_000), networkFinalized(tracker, ups.NetworkId()))

		tracker.SetFinalizedBlockNumber(ups, 9_999)
		assert.Equal(t, int64(9_999), networkFinalized(tracker, ups.NetworkId()))
	})
