...
// Rotate advances every rolling-window component by one sub-bucket.
...
// Reset wipes every counter and the quantile sketch fully. Test/admin helper.
...
func (t *Tracker) getRemoteRateLimitedCounter(up common.Upstream, method, userId string, args ...string) prometheus.Counter {
	finality := ""
	if len(args) == 1 {
		finality = args[0]
	} else if len(args) >= 2 {
...
func (t *Tracker) sweepIdleObservers(cutoffMs int64) {
	t.urdObsCache.Range(func(key, value any) bool {
		...
		telemetry.MetricUpstreamRequestDuration.DeleteLabelValues(
			k.project, k.vendor, k.network, k.upstream, k.category, k.composite, k.finality,
		)
		return true
	})
	...
	t.remoteRateLimitedCounterCache.Range(func(key, value any) bool {
		...
		telemetry.MetricRateLimitsTotal.DeleteLabelValues(
			k.project, k.network, k.vendor, k.upstream, k.category, k.finality, k.user,
...
func (t *Tracker) RecordUpstreamRequest(up common.Upstream, method string, finalities ...common.DataFinalityState) {
...
func (t *Tracker) RecordUpstreamDurationStart(upstream common.Upstream, method string, compositeType string, finality common.DataFinalityState, userIds ...string) *Timer {
	if compositeType == "" {
		compositeType = "none"
	}
	userId := "n/a"
...
func (t *Tracker) RecordUpstreamDuration(up common.Upstream, method string, d time.Duration, isSuccess bool, comp string, finality common.DataFinalityState, userIds ...string) {
	if comp == "" {
		comp = "none"
	}
	userId := "n/a"
...
func (t *Tracker) GetUpstreamMethodMetrics(up common.Upstream, method string, finalities ...common.DataFinalityState) *TrackedMetrics {
	finality := common.DataFinalityStateAll
	if len(finalities) > 0 {
		finality = finalities[0]
	}
