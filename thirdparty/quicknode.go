...
func normalizeQuicknodeEndpoint(chainID int64, rawURL string) string {
	if chainID != QuicknodeHyperevmChainID {
		return rawURL
	}
	...
	if !strings.HasSuffix(trimmedPath, "/evm") {
		return rawURL
	}

	parsedURL.Path = strings.TrimSuffix(trimmedPath, "/evm") + "/nanoreth"
	...
	if upstream.Endpoint == "" {
		...
		for _, endpoint := range endpoints {
			if endpoint.ChainID == chainID && endpoint.HttpUrl != "" {
				...
				upsCopy.Endpoint = normalizeQuicknodeEndpoint(chainID, endpoint.HttpUrl)
