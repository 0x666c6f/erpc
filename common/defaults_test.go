...
func TestSetDefaults_NetworkConfig(t *testing.T) {
	...
	t.Run("NetworkDefaultsDefinesFailsafeNoUserDefinedFailsafe", func(t *testing.T) {
		...
		assert.Equal(t, 100*time.Millisecond, network.Failsafe[0].Timeout.Duration.Resolve(nil))
	...
	t.Run("NetworkDefaultsDefinesHedgeNoUserDefinedFailsafe", func(t *testing.T) {
		...
		assert.Equal(t, 100*time.Millisecond, network.Failsafe[0].Hedge.Delay.Resolve(nil))
		...
		Failsafe: []*FailsafeConfig{
			{
				MatchMethod: "*",
				Timeout: &TimeoutPolicyConfig{
					Duration: NewStaticDuration(9 * time.Second),
				},
			},
		},
			...
			Profiles: map[string]*MethodWorkloadProfileConfig{
				"read-heavy": {
					...
					Failsafe: []*FailsafeConfig{
						{
							Timeout: &TimeoutPolicyConfig{
								Duration: NewStaticDuration(3 * time.Second),
							},
						},
					},
				},
	...
	for i, fs := range network.Failsafe {
		...
		if fs.MatchMethod == "eth_getBalance" {
			...
			if assert.NotNil(t, fs.Timeout) {
				assert.Equal(t, 3*time.Second, fs.Timeout.Duration.Resolve(nil))
			}
		}
...
func TestNetworkConfigSetDefaults_MethodWorkloadProfiles_ExplicitRulePrecedence(t *testing.T) {
		...
		Failsafe: []*FailsafeConfig{
			{
				...
				Timeout: &TimeoutPolicyConfig{
					Duration: NewStaticDuration(1 * time.Second),
				},
			},
			{
				MatchMethod: "*",
	...
	for i, fs := range network.Failsafe {
		...
		if fs.MatchMethod == "eth_getBalance" && fs.Timeout.Duration.Resolve(nil) == time.Second {
			explicitIndex = i
		}
		if fs.MatchMethod == "eth_getBalance" && fs.Timeout.Duration.Resolve(nil) == 3*time.Second {
			profileIndex = i
		}
		if fs.MatchMethod == "*" && wildcardIndex == -1 {
			wildcardIndex = i
...
func TestPostgreSQLValidateRejectsWhitespaceConnectionURI(t *testing.T) {
	t.Parallel()
	...
	err := cfg.Validate()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "connectionUri is required")
}
...
func TestPostgreSQLIAMAuthValidateRejectsIPv6EndpointWithoutPort(t *testing.T) {
	t.Parallel()

	cfg := &PostgreSQLConnectorConfig{
		ConnectionUri: "postgres://erpc-user@[2001:db8::1]:5432/erpc?sslmode=require",
		Table:         "erpc_json_rpc_cache",
		MinConns:      1,
		MaxConns:      2,
		InitTimeout:   Duration(time.Second),
	...
	err := cfg.Validate()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "endpoint must include a port")
}

func TestPostgreSQLIAMAuthValidateRejectsReadonlyIPv6EndpointWithoutPort(t *testing.T) {
	t.Parallel()

	cfg := &PostgreSQLConnectorConfig{
		ConnectionUri:          "postgres://erpc-user@primary.abc123.us-east-1.rds.amazonaws.com:5432/erpc?sslmode=require",
		ReadonlyConnectionUris: []string{"postgres://erpc-user@[2001:db8::1]/erpc?sslmode=require"},
		Table:                  "erpc_json_rpc_cache",
		MinConns:               1,
		MaxConns:               2,
		InitTimeout:            Duration(time.Second),
	...
	err := cfg.Validate()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "readonlyConnectionUris[0] endpoint must include a port")
}
