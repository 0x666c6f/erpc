...
func (p *PostgreSQLConnectorConfig) Validate() error {
	if strings.TrimSpace(p.ConnectionUri) == "" {
	...
	if p.IAMAuth != nil && p.IAMAuth.Enabled {
		...
		if !postgreSQLEndpointHasPort(p.IAMAuth.Endpoint) {
		...
		for i, connectionURI := range append([]string{p.ConnectionUri}, p.ReadonlyConnectionUris...) {
			...
			if i > 0 && parsed.Host != "" && !postgreSQLEndpointHasPort(parsed.Host) {
...
func postgreSQLEndpointHasPort(endpoint string) bool {
...
func (e *EvmNetworkConfig) Validate() error {
	...
	if e.GetLogsMaxDataBytes < 0 {
		return fmt.Errorf("network.*.evm.getLogsMaxDataBytes must be greater than or equal to 0")
	}
